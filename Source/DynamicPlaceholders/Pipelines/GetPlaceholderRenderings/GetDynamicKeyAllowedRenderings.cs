...
namespace DynamicPlaceholders.Pipelines.GetPlaceholderRenderings
{
    public class GetDynamicKeyAllowedRenderings : GetAllowedRenderings
    {
        public new void Process(GetPlaceholderRenderingsArgs args)
        {
            ...
            var regex = new Regex(DynamicPlaceholders.PlaceholderKeyRegex.AllowedRenderingsDynamicKeyRegex);
            var match = regex.Match(currentPlaceholderKey);

            if (match.Success) // && match.Groups.Count > 0)
            {
                //temporaryPlaceholderKey = match.Groups[1].Value;
                temporaryPlaceholderKey = regex.Replace(currentPlaceholderKey, "");
            }
            else
            {
                return;
